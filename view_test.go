...
func TestCheckView(t *testing.T) {
  ...
  views := NewViews(&cache.CacheConfig{Mode: "mutexmap"}, translate, templates) 
  forms := NewForms(&cache.CacheConfig{Mode: "mutexmap"}, translate, templates) 
  forms.Load("./etc.test/")
  views.Load("./etc.test/")
  
  funcs := NewFunctions(translate, forms, views)
  templates.SetFunc(funcs)
  
  assert.Equal(t, int64(1), views.Count())
  ...
  render1, okr := views.Render("table_edit_news", "en_US", "bootstrap4", nil)
